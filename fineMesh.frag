...
void main()
{
	...
	//float map = dot(color, vec3(0, 1, 0)) ;
	...
	float r = texture(texture1, vec2(intercepts * 1.48f)).r;
	float g = texture(texture1,  vec2(intercepts * 1.48f)).g;
	float b = texture(texture1,  vec2(intercepts * 1.48f)).b;
	r *= intensity;
	g *= intensity;
	b *= intensity;
	...
	FragColor = vec4(texture(texture1,tex).rgb,1.0f);
}